...
def dice_loss(input: Tensor, target: Tensor, multiclass: bool = False):
    ...
    target = nn.functional.one_hot(
        target.long(),  num_classes=8).permute(0, 3, 1, 2)
...
def mIoU(gen_mask, input_mask):
    ...
    for i in range(input_mask.shape[0]):
        _, intersection, union = intersectionAndUnion(
            gen_mask[i], input_mask[i])
        area_intersection.append(intersection)
        area_union.append(union)

    IoU = 1.0 * np.sum(area_intersection, axis=0) / \
        np.sum(np.spacing(1)+area_union, axis=0)
